...
import { MapPin, Phone, MessageSquare, Clock, ShieldCheck, Waves, Users, CheckCircle, ChevronLeft, ChevronRight } from "lucide-react";
...
const ResortDetail = () => {
  ...
  const resort = resortsData.find((r) => r.slug === slug);
  const [activeImgIndex, setActiveImgIndex] = useState(0);

  const handlePrev = () => {
    if (resort) {
      setActiveImgIndex((prev) =>
        prev === 0 ? resort.collageImages.length - 1 : prev - 1
      );
    }
  };
  ...
  const handleNext = () => {
    if (resort) {
      setActiveImgIndex((prev) =>
        prev === resort.collageImages.length - 1 ? 0 : prev + 1
      );
    }
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background text-foreground">
        <div className="text-center p-8 bg-card rounded-2xl shadow-luxury">
          <h2 className="text-2xl font-bold mb-4 font-heading">Resort Not Found</h2>
          <p className="text-muted-foreground mb-6">The resort you are looking for does not exist.</p>
          <Link to="/resorts" className="px-6 py-3 bg-primary text-primary-foreground font-semibold rounded-lg">
            Back to Resorts
          </Link>
        </div>
      </div>
  ...
  const breadcrumbSchema = {
    ...
    "itemListElement": [
      ...
      },
      {
        ...
        "position": 3,
        "name": resort.name,
        "item": `https://dandeliwildadventure.com/resorts/${resort.slug}`
      }
    ]
  };
  ...
  const hotelSchema = {
    ...
    "url": `https://dandeliwildadventure.com/resorts/${resort.slug}`,
    "telephone": "+916362908898",
    "email": "info@dandeliwildadventure.com",
    "priceRange": resort.rooms && resort.rooms.length > 0 ? `${resort.rooms[0].price} per person/night` : "INR 1,500 - INR 5,500 per person/night",
    "address": {
      "@type": "PostalAddress",
      "streetAddress": resort.location,
  ...
  const faqSchema = {
    ...
    "mainEntity": [
      {
        "@type": "Question",
        "name": `What is the check-in and check-out time at ${resort.name}?`,
        "acceptedAnswer": {
          "@type": "Answer",
          "text": "We offer a flexible check-in check-out policy (11:30 AM - 10:30 AM) giving you a full 23-hour stay to maximize your adventure time."
        }
      },
      {
        "@type": "Question",
        "name": "Are meals included in the resort packages?",
        "acceptedAnswer": {
          "@type": "Answer",
          "text": "Yes, our packages are all-inclusive of stay, all 3 buffet meals (breakfast, lunch, dinner), evening snacks, and on-site outdoor activities."
        }
      },
      {
        "@type": "Question",
        "name": "What activities are offered at the resort?",
        "acceptedAnswer": {
          "@type": "Answer",
          "text": "We offer a wide range of activities including river water sports (boating, kayaking, zorbing, river swimming) and outdoor fun (archery, bonfire, jungle trekking, indoor sports) based on availability."
        }
      }
    ]
  };
  ...
  const bookingUrl = `/booking?resort=${encodeURIComponent(resort.name)}`;
  const whatsappUrl = `https://wa.me/91${PHONE_NUMBER}?text=Hi%2C%20I%20am%20interested%20in%20booking%20a%20stay%20at%20${encodeURIComponent(resort.name)}%20Dandeli.%20Please%20share%20availability%20and%20packages.`;
  ...
  return (
    <div className="min-h-screen bg-background">
      <SEO
        ...
        keywords={`${resort.name}, resort in Dandeli, best resort in Dandeli, luxury resort in Dandeli, jungle resort in Dandeli, ${resort.name} Dandeli`}
      ...
      <section className="relative py-12 md:py-20 bg-muted/30">
        <div className="container mx-auto px-4">
          ...
          {resort.collageImages && resort.collageImages.length > 0 ? (
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 mb-12">
              ...
              <div className="lg:col-span-4 relative aspect-[16/10] md:h-[500px] w-full overflow-hidden rounded-2xl shadow-luxury group bg-black flex items-center justify-center">
                <AnimatePresence mode="wait">
                  <motion.img
                    key={activeImgIndex}
                    src={resort.collageImages[activeImgIndex]}
                    alt={`${resort.name} view ${activeImgIndex + 1}`}
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                ...
                <div className="absolute bottom-4 left-4 px-4 py-1.5 bg-black/60 backdrop-blur-md rounded-full text-xs font-semibold text-white tracking-widest border border-white/10">
                  {activeImgIndex + 1} / {resort.collageImages.length}
                </div>
              </div>
              ...
              <div className="flex lg:flex-col gap-2.5 overflow-x-auto lg:overflow-y-auto max-h-[500px] scrollbar-thin scrollbar-thumb-primary pb-2 lg:pb-0 pr-1 select-none">
                {resort.collageImages.map((img, idx) => (
                  <button
                    key={idx}
                  ...
                  >
                    <img
                      src={img}
                      alt={`${resort.name} gallery image ${idx + 1}`}
                      className="w-full h-full object-cover"
                      loading="lazy"
                    />
                  </button>
            ...
            <div className="aspect-[16/6] w-full overflow-hidden rounded-2xl shadow-luxury mb-12">
              <img
                ...
                alt={`${resort.name} - Luxury Nature Resort in Dandeli`}
                loading="lazy"
                className="w-full h-full object-cover"
              />
            </div>
          ...
          <div className="bg-card rounded-2xl p-8 md:p-12 shadow-luxury border border-border">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8 pt-8 border-t border-border/60">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h4 className="font-semibold text-foreground">Premium Quality Guaranteed</h4>
                  <p className="text-xs text-muted-foreground">We ensure top-notch hospitality, cleanliness, and safety standards.</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary shrink-0">
          ...
          <div className="container mx-auto px-4 max-w-4xl">
            <div className="text-center mb-10">
              ...
              <h2 className="font-heading text-3xl md:text-4xl font-bold text-foreground">
                Experience {resort.name} <span className="text-gradient-gold">In Motion</span>
              </h2>
            </div>
            
            <div className="relative aspect-[16/9] w-full overflow-hidden rounded-2xl shadow-luxury border border-border/80 bg-black">
              {getYoutubeId(resort.youtubeUrl) ? (
                <iframe
                  className="absolute inset-0 w-full h-full"
                  src={`https://www.youtube.com/embed/${getYoutubeId(resort.youtubeUrl)}`}
                  title={`${resort.name} Video Tour`}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="bg-card rounded-2xl p-8 border border-border shadow-luxury">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h3 className="font-heading text-xl font-bold text-foreground">Resort Water Activities</h3>
                  <p className="text-xs text-muted-foreground">Thrills right in the Kali River waters</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                {resort.riverActivities.map((act) => (
              ...
              {resort.riverActivities.some(act => act.toLowerCase().includes("rafting")) && (
            ...
            <div className="bg-card rounded-2xl p-8 border border-border shadow-luxury">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h3 className="font-heading text-xl font-bold text-foreground">On-Site & Outdoor Fun</h3>
                  <p className="text-xs text-muted-foreground">Standard common activities included with your stay</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                {resort.commonActivities.map((act) => (
                  <div key={act} className="flex items-center gap-2.5 bg-background p-4 rounded-xl border border-border/60">
      ...
      <section className="py-20 bg-background">
        <div className="container mx-auto px-4">
          <div className="text-center mb-14">
            <p className="text-accent font-body text-sm tracking-[0.2em] uppercase mb-2">Affordable Luxury</p>
            <h2 className="font-heading text-3xl md:text-4xl font-bold text-foreground">
              Room Categories & <span className="text-gradient-gold">Tariff Details</span>
            </h2>
            <p className="text-muted-foreground mt-3 max-w-lg mx-auto">
              Every stay package includes spacious accommodation, all 3 buffet meals (breakfast, lunch, dinner), snacks, and outdoor activities.
            </p>
          </div>
          ...
          <div className="mt-14 max-w-2xl mx-auto text-center space-y-6">
            ...
            <p className="text-muted-foreground text-sm">
              Spaces fill up fast, especially for weekends. Book early to secure your preferred room category and enjoy the 23-hour stay (11:30 AM - 10:30 AM) benefit!
            </p>
